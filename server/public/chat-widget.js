;(function () {
  ...
  css.textContent = [
    ...
    /* 录音条 */
    '.gw-rec-bar{display:none;align-items:center;gap:10px;padding:10px 14px;background:#fff3f3;border-radius:10px;margin-bottom:6px;animation:gw-fadein .2s ease;}',
    '.gw-rec-bar.show{display:flex;}',
    '.gw-rec-dot{width:10px;height:10px;border-radius:50%;background:#ef4444;animation:gw-recdot 1s ease-in-out infinite;}',
    '@keyframes gw-recdot{0%,100%{opacity:1;}50%{opacity:.3;}}',
    '.gw-rec-time{font-size:14px;font-weight:600;color:#ef4444;min-width:36px;font-variant-numeric:tabular-nums;}',
    '.gw-rec-cancel{padding:4px 12px;border-radius:6px;border:1px solid #e2e8f0;background:#fff;font-size:12px;color:#64748b;cursor:pointer;margin-left:auto;}',
    '.gw-rec-cancel:hover{background:#f8fafc;border-color:#cbd5e1;}',
    '.gw-rec-stop{padding:4px 12px;border-radius:6px;border:none;background:#ef4444;font-size:12px;color:#fff;cursor:pointer;}',
    '.gw-rec-stop:hover{background:#dc2626;}',

    /* STT 按钮激活态 */
    '.gw-tool-btn.gw-stt-active{color:#ef4444!important;animation:gw-recdot 1s ease-in-out infinite;}',
    '.gw-tool-btn.gw-mic-active{color:#ef4444!important;}',
    ...
    '.gw-tool-btn{width:30px;height:30px;border-radius:6px;border:none;background:transparent;cursor:pointer;color:#94a3b8;display:flex;align-items:center;justify-content:center;transition:all .12s;position:relative;}',
  ...
  win.innerHTML = [
    ...
    '    <button type="button" class="gw-tool-btn" id="gw-mic-btn" title="发送语音"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><path d="M12 1a3 3 0 0 0-3 3v8a3 3 0 0 0 6 0V4a3 3 0 0 0-3-3z"/><path d="M19 10v2a7 7 0 0 1-14 0v-2"/><line x1="12" y1="19" x2="12" y2="23"/><line x1="8" y1="23" x2="16" y2="23"/></svg></button>',
    '    <button type="button" class="gw-tool-btn" id="gw-stt-btn" title="语音转文字"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><path d="M12 1a3 3 0 0 0-3 3v8a3 3 0 0 0 6 0V4a3 3 0 0 0-3-3z"/><path d="M19 10v2a7 7 0 0 1-14 0v-2"/><line x1="12" y1="19" x2="12" y2="23"/><line x1="8" y1="23" x2="16" y2="23"/></svg><span style="position:absolute;right:0;bottom:0;font-size:9px;font-weight:700;line-height:1;">文</span></button>',
    '    <div class="gw-emoji-panel" id="gw-emoji-panel"></div>',
    '  </div>',
    '  <div class="gw-rec-bar" id="gw-rec-bar">',
    '    <span class="gw-rec-dot"></span>',
    '    <span class="gw-rec-time" id="gw-rec-time">0:00</span>',
    '    <button type="button" class="gw-rec-cancel" id="gw-rec-cancel">取消</button>',
    '    <button type="button" class="gw-rec-stop" id="gw-rec-stop">发送</button>',
    '  </div>',
  ...
  var micBtn   = win.querySelector('#gw-mic-btn')
  var sttBtn   = win.querySelector('#gw-stt-btn')
  var recBar   = win.querySelector('#gw-rec-bar')
  var recTime  = win.querySelector('#gw-rec-time')
  var recCancel = win.querySelector('#gw-rec-cancel')
  var recStop  = win.querySelector('#gw-rec-stop')

  /* ========== 事件绑定 ========== */
  closeBtn.onclick = toggleChat
  sendBtn.onclick  = send
  imgBtn.onclick   = function () { fileInput.click() }
  micBtn.onclick   = function () { gwStartRecording() }
  sttBtn.onclick   = function () { gwToggleSTT() }
  recCancel.onclick = function () { gwCancelRecording() }
  recStop.onclick  = function () { gwStopRecording() }
  ...
  /* ========== 语音录制 ========== */
  var gwMediaRecorder = null
  var gwAudioChunks = []
  var gwRecordTimer = null
  var gwRecordSec = 0
  var gwIsRecording = false

  function gwGetAudioMime() {
  ...
  function gwUpdateRecTime() {
    var m = Math.floor(gwRecordSec / 60)
    var s = gwRecordSec % 60
    recTime.textContent = m + ':' + String(s).padStart(2, '0')
  }

  function gwStartRecording() {
    if (gwIsRecording) return
    if (typeof MediaRecorder === 'undefined' || !navigator.mediaDevices) {
      alert('当前浏览器不支持录音功能'); return
    }
    navigator.mediaDevices.getUserMedia({ audio: true }).then(function (stream) {
      gwAudioChunks = []
      gwMediaRecorder = new MediaRecorder(stream, { mimeType: gwGetAudioMime() })
      gwMediaRecorder.ondataavailable = function (e) {
        if (e.data.size > 0) gwAudioChunks.push(e.data)
      }
      gwMediaRecorder.onstop = function () {
        stream.getTracks().forEach(function (t) { t.stop() })
        if (gwAudioChunks.length > 0 && gwRecordSec >= 1) {
          var blob = new Blob(gwAudioChunks, { type: gwMediaRecorder.mimeType })
          gwSendAudioBlob(blob)
        }
        clearInterval(gwRecordTimer)
        gwRecordSec = 0
        gwUpdateRecTime()
      }
      gwMediaRecorder.start()
      gwIsRecording = true
      gwRecordSec = 0
      gwUpdateRecTime()
      recBar.classList.add('show')
      micBtn.classList.add('gw-mic-active')
      gwRecordTimer = setInterval(function () {
        gwRecordSec++
        gwUpdateRecTime()
        if (gwRecordSec >= 60) gwStopRecording()
      }, 1000)
    }).catch(function () {
  ...
  function gwStopRecording() {
    if (gwMediaRecorder && gwMediaRecorder.state !== 'inactive') {
      gwMediaRecorder.stop()
    }
    gwIsRecording = false
    recBar.classList.remove('show')
    micBtn.classList.remove('gw-mic-active')
  }

  function gwCancelRecording() {
    gwAudioChunks = []
    if (gwMediaRecorder && gwMediaRecorder.state !== 'inactive') {
      gwMediaRecorder.onstop = function () {
        if (gwMediaRecorder.stream) gwMediaRecorder.stream.getTracks().forEach(function (t) { t.stop() })
      }
      gwMediaRecorder.stop()
    }
    clearInterval(gwRecordTimer)
  ...
  function gwSendAudioBlob(blob) {
    var reader = new FileReader()
    reader.onload = function () {
      var base64 = reader.result
      appendMsg({ sender_role: 'user', type: 'audio', content: base64, created_at: new Date().toISOString() })
      scrollDown()
      api('POST', '/send', { visitor_id: visitorId, visitor_token: visitorToken, type: 'audio', content: base64 })
        .then(function (d) {
  ...
  /* ========== 语音转文字 (STT) ========== */
  var gwSttRecognition = null
  var gwSttListening = false

  function gwToggleSTT() {
    if (gwSttListening) { gwStopSTT(); return }
    var SR = window.SpeechRecognition || window.webkitSpeechRecognition
    if (!SR) { alert('当前浏览器不支持语音识别，请使用 Chrome'); return }
    gwSttRecognition = new SR()
    gwSttRecognition.lang = 'zh-CN'
    gwSttRecognition.continuous = true
    gwSttRecognition.interimResults = true
    gwSttListening = true
    sttBtn.classList.add('gw-stt-active')
    gwSttRecognition.onresult = function (e) {
      var final = '', interim = ''
    ...
    gwSttRecognition.onerror = function () {
      gwSttListening = false
      sttBtn.classList.remove('gw-stt-active')
    }
    gwSttRecognition.onend = function () {
      gwSttListening = false
      sttBtn.classList.remove('gw-stt-active')
      gwSttRecognition = null
    }
    gwSttRecognition.start()
  }

  function gwStopSTT() {
    if (gwSttRecognition) { gwSttRecognition.stop(); gwSttRecognition = null }
    gwSttListening = false
    sttBtn.classList.remove('gw-stt-active')
  }
  ...
  function toggleChat() {
    ...
    if (isOpen) {
      ...
      stopPoll()
      if (gwIsRecording) gwCancelRecording()
      if (gwSttListening) gwStopSTT()
    }
  }
