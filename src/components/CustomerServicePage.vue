...
async function fetchConversations() {
  try {
    ...
    if (data.code === 0) conversations.value = data.data
  } catch { /* ignore */ }
}
...
<template>
  <div class="cs-page" :class="{ 'mobile-chat-active': mobileShowChat }">
    ...
    <nav class="cs-nav">
      ...
      <div class="nav-links">
        <button type="button" :class="['nav-item', { active: activeNavItem === 'dashboard' }]" @click="handleNavClick('dashboard')">
        ...
        <button type="button" :class="['nav-item', { active: activeNavItem === 'messages' }]" @click="activeNavItem = 'messages'">
        ...
        <button type="button" :class="['nav-item', { active: activeNavItem === 'customers' }]" @click="activeNavItem = 'customers'">
        ...
        <button type="button" :class="['nav-item', { active: activeNavItem === 'analytics' }]" @click="activeNavItem = 'analytics'">
        ...
        <button type="button" :class="['nav-item', { active: activeNavItem === 'settings' }]" @click="activeNavItem = 'settings'">
    ...
    <aside class="cs-sidebar">
      ...
      <div class="conv-list">
        ...
        <div v-else-if="filteredConversations.length === 0" class="conv-empty">
          <svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="#cdd5e0" stroke-width="1.5"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg>
        ...
        <template v-else>
          <div
            ...
            :class="['conv-card', { active: activeConvId === c.id }]"
          ...
          >
            ...
            <div class="conv-body">
              ...
              <div class="conv-bottom">
                ...
                <span v-if="c.unread_count > 0" class="conv-badge">{{ c.unread_count > 99 ? '99+' : c.unread_count }}</span>
              </div>
            </div>
          </div>
        </template>
      ...
      <template v-if="activeConvId && activeConv">
        ...
        <div class="chat-body" ref="chatBody">
          ...
          <template v-for="(msg, idx) in messages" :key="msg.id">
            <div v-if="idx === 0 || (new Date(msg.time) - new Date(messages[idx-1].time)) > 300000" class="time-divider">
            ...
            <div :class="['msg-row', msg.role === 'user' ? 'incoming' : 'outgoing']">
              ...
              <div v-if="msg.role !== 'user'" class="msg-ava outgoing-ava">
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#fff" stroke-width="2.5" stroke-linecap="round"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>
              </div>
            </div>
          </template>
        </div>
        ...
        <footer class="chat-foot">
          ...
          <div class="foot-toolbar">
              ...
              <Transition name="ep">
                <div v-if="showEmoji" class="emoji-panel" @click.stop>
                  <div v-for="g in emojiGroups" :key="g.label" class="eg">
                    ...
                    <div class="eg-grid">
                      <button v-for="e in g.items" :key="e" type="button" class="ei" @click="insertEmoji(e)">{{ e }}</button>
                    </div>
                  </div>
                </div>
              </Transition>
            ...
            <!-- 录音发送语音 -->
            <button type="button" class="tb-btn voice-rec-btn" :class="{ active: recording }" @click="recording ? stopRecording() : startRecording()" title="发送语音消息">
          ...
          <div class="foot-input">
            <input v-model="input" type="text" placeholder="输入回复内容..." @keydown="handleKeydown" @paste="onPaste" :disabled="sending" />
      ...
      <template v-else>
        <div class="no-selection">
          <div class="no-sel-icon">
            <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="#c4cdd8" stroke-width="1.2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 11.5a8.38 8.38 0 0 1-.9 3.8 8.5 8.5 0 0 1-7.6 4.7 8.38 8.38 0 0 1-3.8-.9L3 21l1.9-5.7a8.38 8.38 0 0 1-.9-3.8 8.5 8.5 0 0 1 4.7-7.6 8.38 8.38 0 0 1 3.8-.9h.5a8.48 8.48 0 0 1 8 8v.5z"/></svg>
          </div>
          <h3>选择一个会话</h3>
...
.nav-brand h3 { font-size: 16px; font-weight: 800; color: #1e293b; margin: 0; }
.nav-brand small { font-size: 11px; color: #94a3b8; }

.nav-links { flex: 1; padding: 8px 10px; display: flex; flex-direction: column; gap: 2px; }
...
.create-ticket-btn:hover { background: #1d4ed8; transform: translateY(-1px); }
...
.nav-footer-item:hover { background: #f8fafc; color: #1e293b; }
...
.sidebar-title-row h2 { font-size: 15px; font-weight: 800; color: #1e293b; margin: 0; }
.conv-count-badge {
...
.sidebar-search:focus-within { border-color: #2563eb; background: #fff; box-shadow: 0 0 0 3px rgba(37,99,235,.08); }
...
.conv-list { flex: 1; overflow-y: auto; padding: 8px; scrollbar-width: none; }
...
.conv-empty p { font-size: 14px; color: #647184; font-weight: 600; margin: 4px 0 0; }
.conv-empty small { font-size: 12px; color: #b0b8c6; }
.conv-spinner {
  width: 24px; height: 24px;
  border: 3px solid #edf1f6; border-top-color: #2563eb;
...
.chat-head-info strong { display: block; font-size: 15px; color: #1e293b; }
.head-sub { font-size: 12px; color: #94a3b8; }
.visitor-tag {
  display: inline-block; padding: 1px 6px; border-radius: 4px;
  background: #dcfce7; color: #16a34a; font-size: 10px; font-weight: 700;
...
.time-divider { align-self: center; padding: 8px 0; }
.time-divider span {
...
.incoming-ava { background: linear-gradient(135deg, #2563eb, #3b82f6); color: #fff; }
.outgoing-ava { background: linear-gradient(135deg, #0ea5e9, #6366f1); color: #fff; }
...
.msg-ts {
  font-size: 10px; color: #b0b8c6; padding: 0 4px;
...
.img-bub {
  ...
  box-shadow: none !important; max-width: 220px;
}
.img-bub img {
...
.img-bub img:hover { transform: scale(1.02); }
...
.voice-bub:active { opacity: .7; }

.voice-wave {
  display: flex; align-items: flex-end; gap: 2px; height: 18px;
}
.msg-row.incoming .voice-wave { flex-direction: row-reverse; }
...
.no-selection h3 { font-size: 17px; color: #475569; font-weight: 700; margin: 0; }
.no-selection p { font-size: 13px; color: #94a3b8; margin: 0; }
...
.qa-item span { font-size: 11px; color: #64748b; font-weight: 600; white-space: nowrap; }
...
.tb-btn:hover { background: #f1f5f9; color: #2563eb; }
.tb-btn.active { background: #eff6ff; color: #2563eb; }
...
.eg + .eg { border-top: 1px solid #f1f5f9; padding-top: 8px; margin-top: 4px; }
.eg-label { font-size: 11px; font-weight: 700; color: #94a3b8; margin: 0 0 6px; }
.eg-grid { display: grid; grid-template-columns: repeat(8, 1fr); gap: 2px; }
.ei {
...
.ei:hover { background: #f1f5f9; transform: scale(1.2); }
.ei:active { transform: scale(.9); }

.ep-enter-active { transition: all 160ms cubic-bezier(.22,1,.36,1); }
.ep-leave-active { transition: all 100ms ease-in; }
.ep-enter-from { opacity: 0; transform: translateY(8px) scale(.96); }
.ep-leave-to { opacity: 0; transform: translateY(4px); }
...
.foot-input input {
  ...
  font-size: 14px; color: #1e293b; outline: none; padding: 10px 0;
}
.foot-input input::placeholder { color: #94a3b8; }
...
.send-btn:hover { background: #1d4ed8; transform: translateY(-1px); box-shadow: 0 4px 14px rgba(37,99,235,.35); }
.send-btn:active { transform: scale(.95); }
.send-btn:disabled { opacity: .35; pointer-events: none; box-shadow: none; }
...
.preview-mask img {
  ...
  box-shadow: 0 24px 64px rgba(0,0,0,.4); object-fit: contain;
}
.pf-enter-active, .pf-leave-active { transition: opacity 200ms ease; }
...
.cs-recording-bar {
  display: flex; align-items: center; gap: 8px;
  padding: 8px 24px; font-size: 12px; color: #d4380d;
...
.cs-rec-timer { font-weight: 700; font-family: 'SF Mono', Consolas, monospace; }
.cs-rec-hint { flex: 1; color: #94a3b8; }
.cs-rec-cancel {
  padding: 3px 10px; border-radius: 6px; border: none;
  background: #ffd8d2; color: #d4380d; font-size: 11px; font-weight: 700;
  cursor: pointer; transition: background 150ms ease;
}
.cs-rec-cancel:hover { background: #ffc0b5; }

.cs-stt-bar {
  display: flex; align-items: center; gap: 8px;
  padding: 8px 24px; font-size: 12px; color: #2563eb; font-weight: 600;
...
.cs-stt-stop:hover { background: #bfdbfe; }

/* ============================== 响应式 ============================== */
@media (max-width: 760px) {
  .cs-page { padding: 0; gap: 0; background: #fff; height: calc(100vh - 84px); }
  .cs-sidebar { width: 100%; border-radius: 0; box-shadow: none; }
  .cs-main { display: none; border-radius: 0; box-shadow: none; }
  .cs-page.mobile-chat-active .cs-sidebar { display: none; }
  .cs-page.mobile-chat-active .cs-main { display: flex; }
  .back-btn { display: grid; }
  .chat-head { padding: 12px 14px; }
  .chat-body { padding: 14px; scrollbar-width: none; }
  .chat-body::-webkit-scrollbar { display: none; }
  .msg-row { max-width: 85%; }
  .foot-toolbar { padding: 4px 14px; }
  .foot-input { margin: 0 14px 12px; padding-bottom: calc(4px + env(safe-area-inset-bottom, 0px)); }
  .emoji-panel { width: 280px; }
  .eg-grid { grid-template-columns: repeat(7, 1fr); }
}
</style>
